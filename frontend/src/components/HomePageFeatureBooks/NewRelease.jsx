...
const styles = `
  ...
  /* When no arrows, clip has no flex-grow needed, just wraps content */
...
export default function BookLeaseSection({ books }) {
  ...
  // Whether arrows should be shown on desktop (only when more than visCount books)
  const showArrows = displayBooks.length > visCount;

  // Whether "View More/Less" should be shown on mobile (only when more than 4 books)
  ...
  // Clamp idx when books or visCount change
  ...
  // Mobile: show all 4 initially (2 rows of 2), expand shows rest
  ...
  return (
    <>
      ...
      <section className="bs-section" ref={sectionRef} aria-label="Book lease section">
        ...
        {/* ── MOBILE GRID (< 480px): 2 cards per row, 2 rows (4 cards) initially ── */}
        ...
        {/* Mobile View More / Less — only rendered when there are more than 4 books */}
        {showViewMore && (
          <div
            className="bs-mobile-viewmore"
            style={{ display: "none", justifyContent: "center", width: "100%", marginTop: "20px" }}
          >
            <button
        ...
        {/* ── CAROUSEL (≥ 480px): 3 cards visible, arrows only when > 3 books ── */}
        <div className={`bs-carousel-row${showArrows ? "" : " bs-no-arrows"}`}>
          {/* Left arrow — only rendered when needed */}
          {showArrows && (
          ...
          <div className="bs-clip" ref={clipRef}>
            <div
              className="bs-track"
              style={{ transform: showArrows ? `translateX(-${translateX}px)` : "none" }}
            >
          ...
          {/* Right arrow — only rendered when needed */}
          {showArrows && (
